...
{
    class EnemyManager //pg 132, the enemy manager
    {
        ...
        private PlayerManager playerManager; //reference so it'll shoot at the player

        public int MinShipsPerWave = 5; //enemies will come in waves
        ...
        private float shipShotChance = 0.2f; //eveyr frame, each active ship has a 0.2% of firing

        private List<List<Vector2>> pathWaypoints = //a list of LISTS, sheesh, of Vector2 objects.
            new List<List<Vector2>>();

        private Dictionary<int, int> waveSpawns = new Dictionary<int,
            int>();

        public bool Active = false; //was true, pg 162 makes it false. Structuring the game. 

        private Random rand = new Random();

        private void setUpWaypoints()//pg 134, setting up WayPoints()
        {
        ...
        public EnemyManager( //pg135, constructor
            Texture2D texture,
            ...
            Rectangle screenBounds)
        {
            ...
            setUpWaypoints(); //end pg 135 code
        }

        public void SpawnEnemy(int path) //pg 135 code, spawning enemies
        {
        ...
        public void Update(GameTime gameTime) //pg 137, updating and drawing the EnemyManager
        {
            ...
            for (int x = Enemies.Count - 1; x >= 0; x--)
            {
                ...
                else
                {
                    if ((float)rand.Next(0, 1000) / 10 <= shipShotChance)
                    {
                        ...
                        Vector2 shotDirection = playerManager.playerSprite.Center -
                            fireLoc;
